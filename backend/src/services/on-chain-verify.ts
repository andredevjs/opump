...
import type { LaunchTokenContract, DeploymentTransaction } from '../types/contracts.js';
...
async function getProvider(): Promise<JSONRpcProvider> {
  if (!_cachedProvider) {
    const { JSONRpcProvider: JSONRpcProviderClass } = await import('opnet');
    const { networks } = await import('@btc-vision/bitcoin');
    const network = config.network === 'mainnet' ? networks.bitcoin : networks.opnetTestnet;
    _cachedProvider = new JSONRpcProviderClass({ url: config.opnetRpcUrl, network });
  }
...
export async function verifyTokenOnChain(
  ...
  const deploymentTx = tx as unknown as DeploymentTransaction;
  ...
  try {
    ...
    if (rawDeployer) {
      ...
      if (typeof rawDeployer === 'string') {
        ...
        console.error('[Tokens] Unexpected deployer address type:', typeof rawDeployer);
