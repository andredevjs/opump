...
await opnet('OPumpFactory', async (vm: OPNetUnit) => {
    ...
    await vm.it('should return sequential indices for multiple registrations', async () => {
        const { tokenIndex: idx0 } = await factory.registerToken('A', 'A', 0n, 0n, 0n, 0n, deployer);
        const { tokenIndex: idx1 } = await factory.registerToken('B', 'B', 0n, 0n, 0n, 0n, deployer);
    ...
    await vm.it('should track creator token count', async () => {
        await factory.registerToken('A', 'A', 0n, 0n, 0n, 0n, deployer);
        await factory.registerToken('B', 'B', 0n, 0n, 0n, 0n, deployer);

        const creatorCount = await factory.getTokensByCreator(deployer);
        Assert.expect(creatorCount).toEqual(2n);
    });

    await vm.it('should reject combined allocation > 2500 bps', async () => {
        await Assert.expect(async () => {
            // 1500 + 1100 = 2600 > 2500
            await factory.registerToken('Bad', 'BAD', 1500n, 1100n, 0n, 0n, deployer);
        }).toThrow('Combined allocation exceeds 25%');
    });

    await vm.it('should reject buyTax > 300 bps', async () => {
        await Assert.expect(async () => {
            await factory.registerToken('Bad', 'BAD', 0n, 301n, 0n, 0n, deployer);
        }).toThrow('Buy tax exceeds 3%');
    });

    await vm.it('should reject sellTax > 500 bps', async () => {
        await Assert.expect(async () => {
            await factory.registerToken('Bad', 'BAD', 0n, 0n, 501n, 0n, deployer);
        }).toThrow('Sell tax exceeds 5%');
    });

    await vm.it('should reject invalid flywheel destination > 2', async () => {
        await Assert.expect(async () => {
            await factory.registerToken('Bad', 'BAD', 0n, 0n, 0n, 3n, deployer);
    ...
    await vm.it('should track multiple creators independently', async () => {
        ...
        await factory.registerToken('A', 'A', 0n, 0n, 0n, 0n, deployer);
        await factory.registerToken('B', 'B', 0n, 0n, 0n, 0n, creator2);
